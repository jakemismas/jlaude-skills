...
This is not a feature specâ€"it's a conversation starter that captures *who* benefits, *what* they're trying to do, *why* it matters, and *how* you'll know it works.
...
- **Not a feature list:** "I want dashboards, reports, and analytics" (this is too bigâ€"needs splitting)
...
- When stories are too large (split firstâ€"see `skills/user-story-splitting/SKILL.md`)
...
- **When:** [Event that triggers the actionâ€"aligns with 'I want to']
- **Then:** [Expected outcomeâ€"aligns with 'so that']
```

**Quality checks:**
- **Multiple Givens are okay:** Preconditions stack up (e.g., "Given I'm logged in" + "Given I have items in my cart")
- **Only one When:** If you need multiple "When" statements, you likely have multiple storiesâ€"split them
- **Only one Then:** If you need multiple "Then" statements, you likely have multiple storiesâ€"split them
- **Alignment:** Does "When" match "I want to"? Does "Then" match "so that"?

**Red flags:**
- **Multiple Whens/Thens:** Sign of scope creepâ€"split the story (reference `skills/user-story-splitting/SKILL.md`)
- **Vague Thens:** "Then I see improved performance" (unmeasurableâ€"make it specific)
...
**Fix:** If there's no user outcome, it's not a user storyâ€"use an engineering task or tech debt ticket instead.
...
### Related Skills
- `skills/user-story-splitting/SKILL.md` â€" How to break large stories into smaller ones
- `skills/proto-persona/SKILL.md` â€" Defines the "As a [persona]" section
- `skills/problem-statement/SKILL.md` â€" Stories should address validated problems
- `skills/epic-hypothesis/SKILL.md` â€" Epics decompose into user stories

### Optional Helpers
- `skills/user-story/scripts/user-story-template.py` â€" Deterministic Markdown stub generator (no network access)

### External Frameworks
- Mike Cohn, *User Stories Applied* (2004) â€" Origin of the "As a / I want / so that" format
- Gherkin (Cucumber) â€" "Given/When/Then" acceptance criteria format
